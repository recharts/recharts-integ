...
export function replacePackageVersion(
  ...
  try {
    ...
    return TestOutcome.fail(
      ...
      new Error(`Failed to read ${packageJsonPath}: ${error}`),
    );
  }
  let replaced = false;
  ...
  if (packageJson.dependencies && packageJson.dependencies[dependencyName]) {
    console.log(
      `Replacing ${dependencyName} dependency in ${packageJsonPath} with '${version}'`,
    );
  ...
  if (
    ...
    console.log(
      `Replacing ${dependencyName} peerDependency in ${packageJsonPath} with '${version}'`,
    );
  ...
  if (!replaced) {
    ...
    return TestOutcome.fail(
      "replace-package-version",
      new Error(`Dependency ${dependencyName} not found in ${packageJsonPath}`),
    );
  }
  ...
  try {
    ...
    return TestOutcome.fail(
      "replace-package-version",
      new Error(`Failed to write ${packageJsonPath}: ${error}`),
    );
  }
}
